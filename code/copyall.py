...
while(True):
    try:
        ...
        if (choose == "1"):
            ...
            while True:
                print(
                    "进入文件归档到文件夹中，\n方式1：文件夹名是文件名的子集，比如：（文件夹名：张三， 文件名：张三123.pdf）\n")
                fangshi = input("请输入要使用的方式(输入0返回上一级): \n")
                if fangshi == "1":
                    print("文件夹名为文件名的子集方式进行移动文档")
                    path = input("请输入你要操作的文件夹或者文件夹路径:")
                    try:
                        file2dir(path)
                    ...
                    print("输入有误, 请重新输入")
            # while True:
            #     print("进入文件归档到文件夹中，接下来有两种操作方式：\n方式1：文件夹名是文件名的子集，比如：（文件夹名：张三， 文件名：张三123.pdf）\n"
            #           "方式2:使用相似度匹配的算法: 先计算出要匹配的相似度, 再根据相似度进行匹配 ")
            #     fangshi = input("请输入要使用的方式(输入0返回上一级): \n")
            #     if fangshi == "1":
            #         print("文件夹名为文件名的子集方式进行移动文档")
            #         path = input("请输入你要操作的文件夹或者文件夹路径:")
            ...
            #             print(e)
            #     elif fangshi == "2":
            #         print("按文件名和文件夹名的相似度来移动文件, 还未开发.....下个版本迭代!")
            #
            #     elif fangshi == "0":
            #         break
            #     else:
